...
export default class ResourceList extends Component {
    ...
    columns = [
        ...
        {title: '幼教标题', dataIndex: 'resource_name', key: 'live_title',align: 'center'},
        {
            title: '幼教封面', dataIndex: 'resource_img', key: 'live_img',align: 'center',
            render:text=>{
              return(
        ...
        {title: '所属作者', dataIndex: 'resource_author', key: 'live_begin_time',align: 'center'},
        {title: '所属分类', dataIndex: 'resource_category_id', key: 'live_author',align: 'center'},
        // 焦点switch
        {
    ...
    _onChange(checked,record){
      ...
      setFocusResource(record.id,checked?1:0).then(res=>{
          console.log(res);
        if(res.status===1){
    ...
    _getlistDate(pageNum,pageSize){
        ...
        getResourceList(pageNum,pageSize).then(res=>{
            console.log(res);
            if(res.status===1){
                this.setState({
                    dataSource:res.data.resource_list,
                    total:res.data.resource_count,
                    isLoading:false
                })
            }
        })
